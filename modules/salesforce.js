...
let findTitleCard = name => {
    ...
    return new Promise((resolve, reject) => {
        ...
        let q = "SELECT Id,createddate,Name,Picture_URL__c,Title_Cards__c,Description FROM Account WHERE Title_Cards__c LIKE '%" + name + "%' ORDER BY createddate ASC LIMIT 5";
...
let findOpenBranches = parentaccountid => {
    return new Promise((resolve, reject) => {
        console.log('bfo query');
        let q = "SELECT Id, Name,Picture_URL__c,parentid,parent.name,IsOpen__c,Description FROM Account WHERE Title_Cards__c LIKE '%" + 'Top5' + "%'  ";
